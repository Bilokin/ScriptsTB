...
void basymmetry(string filename = "TTBarProcessorLeft.root",  bool drawCorrection = true, float drawBkg = true)
{
	...
	string cuts = " && Sphericity < 10.2 && InvMass > 180 && maxPhotonEnergy < 40 && B1mass + B2mass < 120 - 9.2*abs(ThrustCos)+1.35*abs(ThrustCos)*abs(ThrustCos) && methodRefused == 0 && methodUsed > 0";// && methodCorrect > 0"; 
	...
	if (drawCorrection) 
	{
		...
		TMatrixDEigen  matrix(r->GetCovarianceMatrix());	
		...
		cout << "0: " << matrix.GetEigenValuesRe()[0]  << " 1: " << matrix.GetEigenValuesRe()[1]  << " 2: " << matrix.GetEigenValuesRe()[2] <<  endl;
	}
	TF1 * freconobkg = NULL;
	if (drawBkg) 
	{
