void topcheck(string filename = "TTBarProcessorLeft.root")
{
	...
	c1->Divide(3,1);
	c1->cd(2);
	TH1F * cosRecoKaon = new TH1F("cosRecoKaon", "Reconstructed with kaons", bin_e,-1.0,max_e);
	TH1F * cosRecoLepton = new TH1F("cosRecoLepton", "Reconstructed with lepton", bin_e,-1.0,max_e);
	TH1F * cosRecoVtx = new TH1F("cosRecoVtx", "Reconstructed with vertex", bin_e,-1.0,max_e);
	cosRecoKaon->Sumw2();
	cosRecoVtx->Sumw2();
	cosRecoLepton->Sumw2();
	...
	cosRecoKaon->SetLineWidth(3);
	cosRecoVtx->SetLineWidth(3);
	cosRecoKaon->SetLineColor(kBlue+1);
	cosRecoVtx->SetLineColor(kOrange);
	cosRecoLepton->SetLineWidth(3);
	...
	int leptons = Stats->Draw("-Top2leptonCharge*Top1costheta >>cosRecoLepton", (cuts + "MCBWcorrect == 1").c_str());
...
{
	TLegend *legendMean2 = new TLegend(0.20,0.75,0.75,0.85,NULL,"brNDC");
        legendMean2->SetFillColor(kWhite);
        legendMean2->SetBorderSize(0);
        legendMean2->AddEntry(cosGen,"Generated","f");
        legendMean2->AddEntry(h,h->GetTitle(),"f");
	legendMean2->Draw();
}
